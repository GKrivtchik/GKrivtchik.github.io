...
I will also call "collapsing" the time mesh the process of reducing the number of time steps while maintaining the total duration. The reflections below are based on [Nosy](https://oecd-nea.github.io/Nosy.jl), which uses the power formalism[^3], under which power (or other flows) are lineary interpolated over the intervals. In particular, for a model under power formalism, the effect of mesh collapsing is quite transparent: a time step longer than one hour will simply imply that power will be linearly interpolated for the duration above one hour. 
